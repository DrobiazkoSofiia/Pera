...
@section('body_content')
    <div class="flex justify-between items-center bg-gray-800 rounded mb-3 text-white">
        <h1 class="font-medium px-2">Edit Menu</h1>
    </div>

    <section class="mt-4">
        <form method="POST" action="{{ route('dpanel.menus.update', $menu->id) }}">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label>Menu Name</label>
                <input type="text" name="name" value="{{ $menu->name }}" placeholder="Enter Menu Name" required
            ...
            <div class="mb-3">
                ...
                <select name="breakfast" required class="w-full px-2 bg-transparent border rounded focus:outline-none">
                    @foreach($meals as $meal)
                        <option value="{{ $meal->id }}" {{ $menu->meals->contains($meal->id) ? 'selected' : '' }}>{{ $meal->name }}</option>
                    @endforeach
                </select>
            </div>
